...
const INITIAL_MSGS = [
  { role: 'system', text: 'Loupe — the AI web-design studio. Describe a site and watch a team of agents build it, then refine on the canvas or by pointing at any element. Type / for commands.' },
...
VERIFY before finishing: set \`data-theme="${mode}"\` and audit EVERY page — every heading, paragraph, label, button, card, nav item, input, icon and section background must have clear contrast (no ${dark ? 'dark-on-dark' : 'light-on-light'}); accents stay vivid; borders & shadows read on the new surfaces; images/logos aren't lost. Fix anything that fails, then confirm.`;
  ...
  useEffect(() => { phCtlRef.current = { lastRef: 0, muted: false, remeasuring: false }; setPageH(0); }, [activeTab, device]); // reset the vh-guard + artboard's reference height on a page SWITCH *or* a DEVICE switch so a viewport-relative (vh/svh) page measures against a fresh one-screen viewport, NOT the previous (taller) page's/device's height (else `ph = max(content, pageH)` only grows → a page inherits a taller page's/mobile device's height when returning to desktop)
